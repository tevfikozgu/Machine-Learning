...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 22 14:50:19 2019
...
#29. satir gidicek yorumdaki ilk kelimeyi alicak stopword mu diye bakiak degilse o kelimenin kokunu alicak ve listenin elemani yapicak sirayla

yorum3 = ' '.join(yorum2) #yorum2 yi stringe donusturdu.
derlem = []
for i in range(1000):
    yorum = re.sub('[^a-zA-Z]',' ',yorumlar['Review'][i]) # ^ not demek boylece yorumlar['Review'][6] burdaki kelimelerde [a-zA-Z] olmayanlarin hepsi ' '(space) oluyor.
    yorum = yorum.lower() #kucuk harf yapti.
    yorum = yorum.split() #kelimeleri liste haline getirdi.
...
X = cv.fit_transform(derlem).toarray() #bagimsiz degisken --- bunda satirlar yorumlar sutunler o kelimenin satirda kac tane gectigi buna sparce matrix deniyor
